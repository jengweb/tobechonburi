...
class gapi {
    ...
    public function __construct($config = array()) {
        if ($config['token'] !== null) {
            ...
            $this->authenticateUser($config['email'], $config['password']);
        }
    }
     ...
     *            OPTIONAL: Start index of results
     * @param Int $max_results
     *            OPTIONAL: Max results returned
     */
    public function requestAccountData($start_index = 1, $max_results = 20) {
        ...
        if (substr($response['code'], 0, 1) == '2') {
            ...
            throw new Exception(
                    'GAPI: Failed to request account data. Error: "' . strip_tags($response['body']) . '"');
        }
    }
     ...
     *            Google Analytics dimensions e.g. array('browser')
     * @param Array $metrics
     *            Google Analytics metrics e.g. array('pageviews')
     * @param Array $sort_metric
     *            OPTIONAL: Dimension or dimensions to sort by e.g.('-visits')
     * @param String $filter
     *            OPTIONAL: Filter logic for filtering results
     * @param String $start_date
     *            OPTIONAL: Start of reporting period
     * @param String $end_date
     *            OPTIONAL: End of reporting period
     * @param Int $start_index
     *            OPTIONAL: Start index of results
     * @param Int $max_results
     *            OPTIONAL: Max results returned
    ...
    public function requestReportData(
            $report_id, $dimensions, $metrics, $sort_metric = null, $filter = null, $start_date = null,
        ...
        $response = $this->httpRequest(
                gapi::report_data_url, $parameters, null, $this->generateAuthHeader());
        ...
        if (substr($response['code'], 0, 1) == '2') {
            ...
            throw new Exception(
                    'GAPI: Failed to request report data. Error: "' . strip_tags($response['body']) . '"');
        }
    }
    ...
    protected function processFilter($filter) {
        ...
        $filter = preg_replace(
                '/(&&\s*|\|\|\s*|^)([a-z]+)(\s*' . $valid_operators . ')/i', '$1ga:$2$3', $filter); // Prefix ga: to metrics and dimensions
        $filter = preg_replace('/[\'\"]/i', '', $filter); // Clear invalid quote characters
        $filter = preg_replace(
                array(
                    '/\s*&&\s*/', '/\s*\|\|\s*/', '/\s*' . $valid_operators . '\s*/'
                ), array(
                    ';', ',', '$1'
    ...
    protected function reportObjectMapper($xml_string) {
        ...
        foreach ($google_results->aggregates->metric as $aggregate_metric) {
            ...
            if (preg_match('/^(\d+\.\d+)|(\d+E\d+)|(\d+.\d+E\d+)$/', $metric_value)) {
                $report_aggregate_metrics[str_replace(
                        'ga:', '', $aggregate_metric->attributes()->name)] = floatval($metric_value);
            } else {
                $report_aggregate_metrics[str_replace(
                        'ga:', '', $aggregate_metric->attributes()->name)] = intval($metric_value);
            }
        }
    ...
    protected function authenticateUser($email, $password) {
        $post_variables = array(
            'accountType' => 'GOOGLE', 'Email' => $email, 'Passwd' => $password,
            'source' => gapi::interface_name, 'service' => 'analytics'
        );
        ...
        if (substr($response['code'], 0, 1) != '2' || !is_array($auth_token) || empty(
                $auth_token['Auth'])) {
            throw new Exception(
                    'GAPI: Failed to authenticate user. Error: "' . strip_tags($response['body']) . '"');
        }
    ...
    protected function httpRequest($url, $get_variables = null, $post_variables = null, $headers = null) {
        ...
        if ($interface == 'curl') {
            ...
            throw new Exception(
                    'Invalid http interface defined. No such interface "' . gapi::http_interface . '"');
        }
    }
